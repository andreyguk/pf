Ext.define('pf.ux.grid.FilterBar', {
	extend : 'Ext.AbstractPlugin',
	alias : 'plugin.filterbar',
	uses : [ 'Ext.window.MessageBox', 'pf.ux.form.field.ClearButton', 'pf.ux.form.field.OperatorButton', 'Ext.container.Container', 'Ext.util.DelayedTask', 'Ext.layout.container.HBox', 'Ext.data.ArrayStore', 'Ext.button.Button', 'Ext.form.field.Text', 'Ext.form.field.Number', 'Ext.form.field.Date', 'Ext.form.field.ComboBox' ],
	mixins : {
		observable : 'Ext.util.Observable'
	},

	updateBuffer : 1000, // buffer time to apply filtering when
	// typing/selecting

	columnFilteredCls : Ext.baseCSSPrefix + 'column-filtered', // CSS class to
	// apply to the
	// filtered
	// column header

	renderHidden : true, // renders the filters hidden by default, use in
	// combination with showShowHideButton
	showShowHideButton : true, // add show/hide button in actioncolumn header
	// if found, if not a new small column is
	// created
	...
	showClearButton : true, // use Ext.ux.form.field.ClearButton to allow user
	// to clear each filter, the same as
	// showShowHideButton
	showClearAllButton : true, // add clearAll button in actioncolumn header if
	// found, if not a new small column is created
	clearAllButtonIconCls : 'clear-filters', // css class with the icon of
	// the clear all button
	clearAllButtonTooltip : 'Clear all filters', // button tooltip

	autoStoresRemoteProperty : 'autoStores', // if no store is configured for
	// a combo filter then stores
	// are created automatically, if
	// remoteFilter is true then use
	// this property to return
	// arrayStores from the server
	autoStoresNullValue : '###NULL###', // value send to the server to expecify
	// null filter
	autoStoresNullText : '[empty]', // NULL Display Text
	autoUpdateAutoStores : false, // if set to true combo autoStores are
	// updated each time that a filter is
	// applied

	enableOperators : true, // enable operator selection for number and date
	// filters
	...
	setup : function(grid) {
		...
		me.parseInitialFilters(); // sets me.filterArray with the store
		// previous filters if any (adds operator
		// and type if missing)
	...
	overrideProxy : function() {
		...
		// override encodeFilters to append type and operator in remote
		// filtering
	...
	parseFiltersConfig : function() {
		...
		Ext.each(columns, function(column) {
			if (column.filter) {
				if (column.filter === true || column.filter === 'auto') { // automatic
					// types
					// configuration
					// (store
					// based)
				...
				if (Ext.isString(column.filter)) {
					column.filter = {
						type : column.filter
					// only set type to then use templates
					};
				}
				if (column.filter.type) {
					column.filter = Ext.applyIf(column.filter, me[column.filter.type + 'Tpl']); // also
					// use
					// templates
					// but
					// with
		...
		Ext.each(me.grid.headerCt.getGridColumns(), function(column) {
			if (!Ext.Array.contains(excludedCols, column.id)) {
				column.setPadding = Ext.Function.createInterceptor(column.setPadding, function(h) {
					if (column.hasCls(Ext.baseCSSPrefix + 'column-header-checkbox')) { // checkbox
						// column
	...
	applyFilters : function(field) {
		if (!field.isValid())
			return;
		var me = this, grid = me.grid, column = me.columns.get(field.dataIndex), newVal = (grid.store.remoteFilter ? field.getSubmitValue() : field.getValue());
		...
		if (!Ext.isEmpty(newVal)) {
			if (!grid.store.remoteFilter) {
				var operator = field.operator || column.filter.operator, filterFn;
				switch (operator) {
				case 'eq':
