 ...
 * 
 * Plugin for text components that shows a "operator" button over the text field
 * and shows a menu with operators. For use in Ext.ux.grid.FilterBar plugin
 * 2012-07-30 by Ing. Leonardo D'Onofrio (leonardo_donofrio at hotmail.com)
 */
Ext.define('pf.ux.form.field.OperatorButton', {
	...
	onFieldRender : function(textField) {
		var me = this, bodyEl = me.textField.bodyEl, btn;
	...
	onFieldMouseOver : function(e) {
		...
		if (e.getRelatedTarget() == me.operatorButtonEl.dom) {
			// Moused moved to operator button and will generate another mouse
			// event there.
			// Handle it here to avoid duplicate updates (else animation will
			// break)
	...
	onFieldMouseOut : function(e) {
		...
		if (e.getRelatedTarget() == me.operatorButtonEl.dom) {
			// Moused moved from operator button and will generate another mouse
			// event there.
			// Handle it here to avoid duplicate updates (else animation will
			// break)
	...
	onButtonMouseOut : function(e) {
		...
		e.stopEvent();
		if (me.textField.bodyEl.contains(e.getRelatedTarget())) {
			// will be handled in handleMouseOverInputField() to prevent double
			// update
			return;
		}
		me.operatorButtonEl.removeCls(me.operatorButtonCls + '-mouse-over-button');
	...
	onOperatorClick : function(item) {
		var me = this, btn = me.operatorButtonEl, field = me.textField, lastOperator = me.operator;
	...
	shouldButtonBeVisible : function() {
		...
		if (me.autoHide && !me.menu.isVisible() && !me.fieldInFocus && !me.operatorButtonEl.hasCls(me.operatorButtonCls + '-mouse-over-button') && !me.operatorButtonEl.hasCls(me.operatorButtonCls + '-mouse-over-input')) {
			return false;
		}
		return true;
	},
	...
	updateOperatorButtonVisibility : function() {
		var me = this, btn = me.operatorButtonEl, oldVisible = btn.isVisible(), newVisible = me.shouldButtonBeVisible(), padding;
		...
		me.task.delay(200, function() {
			var oldVisible = btn.isVisible(), newVisible = me.shouldButtonBeVisible();
	...
	repositionOperatorButton : function() {
		var me = this, btn = me.operatorButtonEl;
